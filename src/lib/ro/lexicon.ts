...
import type { RoTuning } from "./tuning";
import { fakeAdjective, fakeNounLemma, fakeStem } from "./fakeWord";

export type RoLexicon = {
...
function maxGraphemesForBuild(
  ...
  const base = m?.language === "ro" ? m.maxWordGraphemes : 14;
  return Math.max(
    5,
    Math.min(16, Math.floor(base * t.shape.spellingCapScale)),
  );
}
...
function uniqueFill(
  ...
  while (out.length < target && guard++ < limit) {
    ...
    if (!validateRoContentWord(m, w, { maxGraphemes: maxG })) continue;
    seen.add(w);
    out.push(w);
  }
  guard = 0;
...
export function buildRoLexicon(
  seed: number,
  t: RoTuning,
  m: CorpusRuntimeModel | null,
  ...
  const nouns = uniqueFill(
    rN,
    (rng) => {
      if (m && m.language === "ro" && rng() < pb) {
        const w = genWordFromCorpus(m, rng, 3, maxG);
        if (validateRoContentWord(m, w, { maxGraphemes: maxG })) return w;
      }
